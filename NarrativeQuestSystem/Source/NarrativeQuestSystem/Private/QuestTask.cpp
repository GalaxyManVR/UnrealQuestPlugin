...
void UQuestTask::BeginTask(UNarrativeComponent* NarrativeComp)
{
	bIsActive = true;
	OwningComp = NarrativeComp;
	TimeSinceLastTick = 0.0f;

	// FIXED: Initialize progress from existing completed data tasks
	if (NarrativeComp && !TaskName.IsNone())
	{
		int32 CompletedCount = NarrativeComp->GetDataTaskCount(TaskName, TaskArgument);
...
void UQuestTask::UpdateTaskProgress(UNarrativeComponent* NarrativeComp)
{
	if (!bIsActive || !NarrativeComp || TaskName.IsNone())
	{
		return;
	}

	// Check the NarrativeComponent for task completion
	int32 CompletedCount = NarrativeComp->GetDataTaskCount(TaskName, TaskArgument);
