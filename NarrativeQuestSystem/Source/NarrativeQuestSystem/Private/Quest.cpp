...
void UQuest::PreSave(FObjectPreSaveContext SaveContext)
{
	...
	// Ensure we have the three required special states
	...
	// Warn if missing required states
	if (!bHasStart)
	{
...
// Add this to Quest.cpp after PreSave
...
void UQuest::Initialize(UNarrativeComponent* InOwningComp)
{
	...
	for (UQuestBranch* Branch : QuestBranches)
	{
		if (Branch)
		{
			...
			// CRITICAL FIX: Removed the code that was re-populating OutgoingBranches/IncomingBranches
			// These arrays are now serialized and should already be populated from the asset
			// Only do this if the arrays are actually empty (for backwards compatibility)
